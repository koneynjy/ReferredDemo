...
void ComputeDirectionalLight(float4 amb, float4 diff, float spec, float gloss, DirectionalLight L,
	float3 normal, float3 toEye,
	out float4 ambient,
	out float4 diffuse,
	out float4 specular)
{
	...
	{
		float3 v = reflect(-lightVec, normal);
		float specFactor = pow(max(dot(v, normalize(toEye)), 0.0f), gloss);

		diffuse = diffuseFactor * diff * L.Diffuse;
		specular = spec * specFactor * L.Specular;
	}
}
